 ...
 */
function mochaStatsCollector(runner: any) {
  const stats = {
    suites: 0,
    tests: 0,
    passes: 0,
  ...
  runner.once("start", () => ((stats as any).start = new Date()));
  runner.once("end", function() {
    (stats as any).end = new Date();
    (stats as any).duration = (stats as any).end - (stats as any).start;
  });
}
